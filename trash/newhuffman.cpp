...
unsigned char node::get_frequency() const {
    return frequency;
}
...
void write_to_file_binary(const std::string& f_in_path, const std::string& f_out_path, std::vector<std::pair<unsigned char, std::vector<unsigned char>>>& codes, int number_of_ch, std::uint32_t& total_aux, std::uint32_t& total_coded) {
...
//gets binary representation of integer
...
void print_sorted_codes(const std::vector<std::pair<unsigned char, std::vector<unsigned char>>>& codes) {
    for (const auto& pair: codes) {
        ...
        std::cout << " " << static_cast<int>(pair.first) << std::endl;;
    }
}
...
std::cout << "Freq: " << (int)e.first << " " << (int)e.second << std::endl; 
}
...
std::vector<std::pair<unsigned char, std::vector<unsigned char>>> get_sorted_codes(const std::string& fin_path, int& number_of_ch, bool& flag, std::uint32_t& total_input) {
    std::vector<std::pair<unsigned char, std::vector<unsigned char>>> sorted_codes;
