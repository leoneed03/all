...
void logger::worker_thread_func(int id, task* new_task) {
	...
	{
		//cv1.notify_all();
		//std::cout<<"new worker "<<size<<std::endl;
		std::unique_lock<std::mutex> u_lock(mtx_queue);
		
		//std::cout<<"new worker got first "<<size<<std::endl;
		//std::unique_lock<std::mutex> g_lock(mtx_size);
		
		//std::cout<<"new worker got second "<<size<<std::endl;
		if (size >= max_size) {
			/*u_lock.unlock();
			...
			//cv1.notify_one();
			//std::cout<<"worker notified "<<size<<std::endl;
			u_lock.unlock();
			...
			//std::cout<<"worker passed waiting "<<size<<std::endl;
