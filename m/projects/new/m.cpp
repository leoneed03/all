...
void calculate_minor( std::vector< std::vector <int> > matrix, int&res ) {
	int size = matrix.size();
	if ( size == 1 ) {
		res = matrix[0][0];
        return;
	...
	for ( int i = 0; i < size; i++ ) {
	    ...
	    if ( i % 2 == 0 ) {
			int t1=0;
			calculate_minor(min,t1);
            res += t1 * matrix[0][i];
		} 
       else {
			int t2 = 0;  
			calculate_minor(min, t2);
            res -= t2 * matrix[0][i];
	   }
    }
}
void calculate_one_thread(std::vector< std::vector <int> > matrix, int &res) {
    ...
    for ( int i = 0; i < size; i++ ) {
        get_minor(matrix, min, 0, i);
        calculate_minor(min, std::ref(v[i]) );
    }
    res = 0;
    for ( int i = 0; i < size; i++ ) {
        if ( i % 2 == 0 )
...
void calculate_multi_thread(std::vector< std::vector <int> > matrix, int &res) {
    ...
    for ( int i = 0; i < size; i++ ) {
        get_minor(matrix, min, 0, i);
        my_threads[i] = std::thread ( calculate_minor, min, std::ref(v[i]));
    }
    for ( int i = 0; i < size; i++ ) {
        my_threads[i].join();
    }
