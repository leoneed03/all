...
void calculate_everything(const std::vector< std::vector <int>> &matrix, int &res, int i) {
    int size  = matrix.size();
    std::vector< std::vector <int>> min(size - 1, std::vector<int>(size - 1, 0) );
    get_minor(matrix, min, 0, i);
    calculate_minor(min, res);
std::cout<<"MINOR: "<<res<<std::endl;
}
void calculate_multi_thread(const std::vector< std::vector <int>> &matrix, int &res) {
    ...
    std::vector<int> v(size, 0);
    for ( int i = 0; i < size; i++ ) {
        my_threads[i] = std::thread (calculate_everything, matrix, std::ref(v[i]), i);
    }
    for ( int i = 0; i < size; i++ ) {
        my_threads[i].join();
    }
